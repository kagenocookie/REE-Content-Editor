...
public partial class EditorWindow : WindowBase, IWorkspaceContainer
{
    ...
    protected void ShowGameSelectionMenu()
    {
        ...
        if (ImGui.BeginMenu(Lang.Home.ActiveGame.Format((string.IsNullOrEmpty(curGame) ? "--" : curGame.ToUpper())))) {
            ...
            if (env != null) {
                if (ImGui.BeginMenu(Lang.Home.ActivePlatform.Format(env.Config.Platform.ToString(), AppIcons.SI_Blank))) {
                    foreach (var otherPlat in PlatformIdentifier.GetAvailableDesktopPlatforms(env.Config.Game)) {
                        if (ImGui.Selectable(Lang.General.BlankPrefix.Format(otherPlat.ToString()), otherPlat == env.Config.Platform)) {
            ...
            foreach (var (game, configured) in games) {
                if (configured && fullSupportedGames.Contains(game)) {
                    if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.TranslateGame(game)))) SetWorkspace(game, null);
                }
            }
            ImGui.Separator();
            foreach (var (game, configured) in games) {
            ...
            ImGui.Separator();
            if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Buttons.ConfigureGames))) {
                AddUniqueSubwindow(new SettingsWindowHandler());
            }
            if (workspace != null && !string.IsNullOrEmpty(workspace.Env.Config.GamePath) && ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Buttons.Open_GameFolder))) {
        ...
        if (ImGui.BeginMenu(Lang.Home.NamedBundle.Format(workspace?.CurrentBundle?.Name ?? "--"), enabled: workspace != null)) {
            ...
            if (ImGui.BeginMenu(Lang.Home.ActiveBundle.Format(workspace.Data.ContentBundle ?? "", AppIcons.SI_Blank))) {
                if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Buttons.NewBundle))) {
                    ShowBundleManagement();
                }
                if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Buttons.NewBundleFromPAK))) {
                ...
                if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Buttons.NewBundleFromLoose))) {
            ...
            if (workspace.CurrentBundle != null) {
                ...
                if (!string.IsNullOrEmpty(textEditor) && ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Buttons.Open_CurrentBundleTextEditor))) {
                    FileSystemUtils.OpenInExternalEditor(workspace.BundleManager.GetBundleFolder(workspace.CurrentBundle), textEditor);
                }
                if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Buttons.BundleFileRescan))) {
                    workspace.RescanFilesInBundle(workspace.CurrentBundle);
                }
                if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Buttons.BundlePublish))) {
    ...
    public void ShowLaunchGameMenu()
    {
        ...
        if (ImGui.BeginMenu($"{AppIcons.SI_Small_ArrowDown}")) {
            ...
            if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Home.LaunchGame_LoosePatch), launchType == GameLaunchType.LooseFiles)) {
            ...
            if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Home.LaunchGame_PakPatch), launchType == GameLaunchType.Pak)) {
    ...
    protected void ShowMainMenuBar()
    {
        ...
        if (ImGui.BeginMenu(Lang.Home.Menu_File)) {
            ImGui.PushStyleVarY(ImGuiStyleVar.ItemSpacing, ImGui.GetStyle().ItemSpacing.Y * 1.5f);
            if (ImGui.BeginMenu(Lang.General.BlankPrefix.Format(Lang.Home.Menu_CreateNew), workspace != null)) {
            ...
            ImGui.Separator();
            if (AppImguiHelpers.HotkeyMenuItem(Lang.General.BlankPrefix.Format(Lang.Home.Menu_Open), AppConfig.Instance.Key_Open.Get(), workspace != null)) {
                ShowFileOpenDialog();
            }
            if (workspace != null) {
                ImGui.BeginDisabled(!hasUnsavedFiles);
                if (AppImguiHelpers.HotkeyMenuItem(Lang.General.BlankPrefix.Format(Lang.Home.Menu_SaveAll), AppConfig.Instance.Key_Save.Get())) {
                ...
                if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Home.Menu_RevertAll))) {
                ...
                if (ImGui.BeginMenu(Lang.General.BlankPrefix.Format(Lang.Home.Menu_OpenedFiles))) {
                    ...
                    if (!files.Any()) {
                        ...
                        ImGui.InputTextWithHint(Lang.General.FilterInput, $"{AppIcons.SI_GenericMagnifyingGlass}", ref openFileFilter, 128);
                        ImGui.Separator();
                ...
                if (ImGui.BeginMenu(Lang.General.BlankPrefix.Format(Lang.Home.Menu_RecentFiles))) {
                    var recents = AppConfig.Settings.RecentFiles;
                    if (recents == null || recents.Count == 0) {
                        ImGui.MenuItem(Lang.Home.RecentFiles_None, false);
                    } else {
                        AppImguiHelpers.ClearableInputText("##Filter"u8, Lang.Home.Hint_SearchRecentFiles.ToString(), ref recentFileFilter, 128);
                        ImGui.SameLine();
                        if (ImGui.Button($"{AppIcons.SI_GenericClear}")) {
                            recents.Clear();
                        }
                        ImguiHelpers.Tooltip(Lang.Buttons.ClearRecent);
                        ImGui.Separator();
                        int i = 0;
            ...
            ImGui.Separator();
            if (AppImguiHelpers.HotkeyMenuItem(Lang.General.BlankPrefix.Format(Lang.Buttons.Exit), new KeyBinding(ImGuiKey.F4, alt: true))) {
                _window.Close();
            }
            ImGui.PopStyleVar();
            ImGui.EndMenu();
        ...
        if (ImGui.BeginMenu(Lang.Home.Menu_Edit)) {
            ...
            if (AppImguiHelpers.HotkeyMenuItem(Lang.Buttons.Undo, AppConfig.Instance.Key_Undo.Get())) UndoRedo.Undo(this);
            ImGui.EndDisabled();

            ImGui.BeginDisabled(!UndoRedo.CanRedo(this));
            if (AppImguiHelpers.HotkeyMenuItem(Lang.Buttons.Redo, AppConfig.Instance.Key_Redo.Get())) UndoRedo.Redo(this);
            ImGui.EndDisabled();
            ...
            ImGui.Separator();
            if (workspace != null) {
                if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Windows.RetargetDesigner))) {
            ...
            if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Tools.FileTester))) {
                AddUniqueSubwindow(new FileTesterWindow());
            }
            if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Tools.IconList))) {
                AddUniqueSubwindow(new IconListWindow());
            }
            ImGui.Separator();
            if (workspace != null && ImGui.BeginMenu(Lang.General.BlankPrefix.Format(Lang.Home.Menu_DataGeneration))) {
                if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Tools.DataGeneration_RebuildRSZ))) {
                ...
                if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Tools.DataGeneration_RebuildEFX))) {
                    if (!runningRszInference) {
                        runningRszInference = true;
                ...
                if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Tools.DataGeneration_ExtensionCache))) {
                    try {
                ...
                if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Tools.DataGeneration_ListFile))) {
            ...
            if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Tools.DumpTranslations))) {
            ...
            if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Tools.DumpLuaTypes))) {
        ...
        if (ImGui.BeginMenu(Lang.Home.Menu_Windows)) {
            ImGui.PushStyleVarY(ImGuiStyleVar.ItemSpacing, ImGui.GetStyle().ItemSpacing.Y * 1.5f);
            if (ImGui.MenuItem(Lang.General.BlankPrefix.Format(Lang.Buttons.NewWorkspace))) {
                UI.OpenWindow(workspace);
            }
            ImGui.Separator();
            if (workspace != null) {
                if (AppImguiHelpers.HotkeyMenuItem(Lang.Windows.PakBrowser, AppConfig.Instance.Key_OpenPakBrowser.Get())) {
                    AddSubwindow(new PakBrowser(workspace, null));
                }
                if (ImGui.MenuItem(Lang.Windows.BundleManager)) {
                    ShowBundleManagement();
        ...
        ImguiHelpers.VerticalSeparator();
        ImGui.BeginDisabled(!UndoRedo.CanUndo(this));
        if (ImGui.MenuItem($"{AppIcons.SI_Undo}")) UndoRedo.Undo(this);
        ImguiHelpers.Tooltip(Lang.Home.Tooltip_Undo.Format(AppImguiHelpers.FormatHotkeyString(AppConfig.Instance.Key_Undo.Get())));
        ImGui.EndDisabled();

        ImGui.BeginDisabled(!UndoRedo.CanRedo(this));
        if (ImGui.MenuItem($"{AppIcons.SI_Redo}")) UndoRedo.Redo(this);
        ImguiHelpers.Tooltip(Lang.Home.Tooltip_Redo.Format(AppImguiHelpers.FormatHotkeyString(AppConfig.Instance.Key_Redo.Get())));
        ImGui.EndDisabled();
