...
public static partial class WindowHandlerFactory
{
    ...
    public static IWindowHandler? CreateFileResourceHandler(ContentWorkspace env, FileHandle file)
    {
        switch (file.Format.format) {
            ...
            case KnownFileFormats.Clip:
                return new TmlEditor<ClipFile>(env, file);
            case KnownFileFormats.Timeline:
                return new TmlEditor<TmlFile>(env, file);
    ...
    public static IObjectUIHandler CreateRSZFieldElementHandlerRaw(UIContext context, RszField field, out FieldConfig? fieldConfig, out ClassConfig? patchConfig)
    {
        ...
        return context.uiHandler = field.type switch {
            ...
            RszFieldType.S16 => TryCreateEnumHandler(ws, field.array ? RszInstance.GetElementType(field.original_type) : field.original_type) ?? new NumericFieldHandler<short>(ImGuiNET.ImGuiDataType.S16),
            RszFieldType.S32 => TryCreateEnumHandler(ws, field.array ? RszInstance.GetElementType(field.original_type) : field.original_type) ?? new NumericFieldHandler<int>(ImGuiNET.ImGuiDataType.S32),
            RszFieldType.S64 => TryCreateEnumHandler(ws, field.array ? RszInstance.GetElementType(field.original_type) : field.original_type) ?? new NumericFieldHandler<long>(ImGuiNET.ImGuiDataType.S64),
            RszFieldType.U8 or RszFieldType.UByte => new NumericFieldHandler<byte>(ImGuiNET.ImGuiDataType.U8),
            RszFieldType.U16 => TryCreateEnumHandler(ws, field.array ? RszInstance.GetElementType(field.original_type) : field.original_type) ?? new NumericFieldHandler<ushort>(ImGuiNET.ImGuiDataType.U16),
            RszFieldType.U32 => TryCreateEnumHandler(ws, field.array ? RszInstance.GetElementType(field.original_type) : field.original_type) ?? new NumericFieldHandler<uint>(ImGuiNET.ImGuiDataType.U32),
            RszFieldType.U64 => TryCreateEnumHandler(ws, field.array ? RszInstance.GetElementType(field.original_type) : field.original_type) ?? new NumericFieldHandler<ulong>(ImGuiNET.ImGuiDataType.U64),
